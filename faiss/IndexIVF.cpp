...
void IndexIVF::boundary_search_v1(
        idx_t n,
        const float* x,
        idx_t k,
        float lower,
    ...
    auto sub_search_func = [this, k, nprobe, params](
        ...
        boundary_search_preassigned_v1(
                                        n,
                                        x,
                                        k,
                                        lower,
...
void IndexIVF::boundary_search_preassigned_v1(
        idx_t n,
        const float* x,
        idx_t k,
        float lower,
        ...
        auto scan_one_list = [&](idx_t key,
            ...
            try {
                if (invlists->use_iterator) {
                    ...
                    nheap += scanner->iterate_codes_boundary_v1(
                            it.get(), simi, idxi, k, list_size, lower, upper);
                    ...
                    nheap += scanner->scan_codes_boundary_v1(
                            list_size, codes, ids, simi, idxi, k, lower, upper);
        ...
        auto scan_list_func = [&](size_t i, size_t ik, RangeQueryResult& qres) {
            ...
            try {
                ...
                if (invlists->use_iterator) {
                    ...
                    scanner->iterate_codes_boundary(
                            it.get(), lower, upper, qres, list_size);
                } else {
                    InvertedLists::ScopedCodes scodes(invlists, key);
                    InvertedLists::ScopedIds ids(invlists, key);
                    list_size = invlists->list_size(key);

                    scanner->scan_codes_boundary(
                            list_size, scodes.get(), ids.get(), lower, upper, qres);
                }
                nlistv++;
                ndis += list_size;
            } catch (const std::exception& e) {
...
bool condition_boundary(float a, float b, float dis){
    float max;
    float min;
    if(a>b){
        max = a;
    ...
    if(dis>=min && dis<=max) return true;

    return false;
}


size_t InvertedListScanner::scan_codes_boundary_v1(
        size_t list_size,
        const uint8_t* codes,
        const idx_t* ids,
        float* simi,
    ...
    if (!keep_max) {
        for (size_t j = 0; j < list_size; j++) {
            float dis = distance_to_code(codes);
            if(condition_boundary(lower, upper, dis)){
                if (dis < simi[0]) {
                    int64_t id = store_pairs ? lo_build(list_no, j) : ids[j];
                    maxheap_replace_top(k, simi, idxi, dis, id);
                    nup++;
...
size_t InvertedListScanner::iterate_codes_boundary_v1(
        InvertedListsIterator* it,
        float* simi,
        idx_t* idxi,
        size_t k,
...
void InvertedListScanner::scan_codes_boundary(
        size_t list_size,
        const uint8_t* codes,
        const idx_t* ids,
        float lower,
    ...
    for (size_t j = 0; j < list_size; j++) {
        ...
        bool keep = !keep_max
                ? (dis>=lower && dis<=upper)
                : (dis<lower || dis>upper); // TODO templatize to remove this test
        if (keep) {
            int64_t id = store_pairs ? lo_build(list_no, j) : ids[j];
            res.add(dis, id);
        }
...
void InvertedListScanner::iterate_codes_boundary(
        InvertedListsIterator* it,
        float lower,
        float upper,
        RangeQueryResult& res,
